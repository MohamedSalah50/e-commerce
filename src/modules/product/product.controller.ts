...
import { ProductParamsDto, UpdateProductAttachmentsDto, UpdateProductDto } from './dto/update-product.dto';
import type { UserDocument } from 'src/db';
import { FilesInterceptor } from '@nestjs/platform-express';
import { cloudFileUpload, fileValidation } from 'src/utils/multer';
import { auth, GetAllDto, GetAllResponse, IProduct, IResponse, storageEnum, User } from 'src/common';
...
export class ProductController {
  ...
  async update(@Param() params: ProductParamsDto,
    @Body() updateProductDto: UpdateProductDto, @User() user: UserDocument): Promise<IResponse<productResponse>> {
    const updatedProduct = await this.productService.update(params.productId, updateProductDto, user);
    return successResponse<productResponse>({ data: { product: updatedProduct } })
  }

  @UseInterceptors(FilesInterceptor('attachments', 5, cloudFileUpload({ validation: fileValidation.image, storageAppraoch: storageEnum.disk })))
  @auth(endpoint.create)
  @Patch(':productId/attachments')
  async updateAttchments(@Param() params: ProductParamsDto,
    @Body() updateProductAttachmentsDto: UpdateProductAttachmentsDto, @User() user: UserDocument
    , @UploadedFiles(new ParseFilePipe({ fileIsRequired: false })) Files: Express.Multer.File[]
  ): Promise<IResponse<productResponse>> {
    const productAttachments = await this.productService.updateAttchments(params.productId, updateProductAttachmentsDto, user, Files);
    return successResponse<productResponse>({ data: { product: productAttachments } })
  }
  ...
  @Get("/archived")
  async findAllArchived(@Query() query: GetAllDto): Promise<IResponse<GetAllResponse<IProduct>>> {
  ...
  async findOneArchived(@Param() params: ProductParamsDto): Promise<IResponse<productResponse>> {
  ...
  async remove(@Param() params: ProductParamsDto, @User() user: UserDocument): Promise<IResponse> {
  ...
  async addToWishlist(@Param() params: ProductParamsDto, @User() user: UserDocument): Promise<IResponse<productResponse>> {
  ...
  async removeFromWishlist(@Param() params: ProductParamsDto, @User() user: UserDocument): Promise<IResponse> {
